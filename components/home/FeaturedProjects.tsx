...
const FeaturedProjects: React.FC<FeaturedProjectsProps> = ({
  ...
  return (
    <div className="grid grid-cols-1">
      ...
      {featuredProjects.map((project) => (
        ...
        >
          <div className="flex justify-between items-start mb-4">
            <h4 className="text-2xl font-bold group-hover:text-purple-500 transition-colors">{project.title}</h4>
            <span className="opacity-0 group-hover:opacity-100 transition-opacity">
              <ArrowUpRight className="w-5 h-5" />
            </span>
          </div>
          <p className={`${mutedText} mb-6`}>{project.description}</p>
      ...
      >
        <div>
          <h4 className="text-xl font-bold font-serif italic mb-1">View All Projects</h4>
