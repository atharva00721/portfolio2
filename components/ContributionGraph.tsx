    ...
    <div className="w-full ">
        ...
        {calendar.weeks.slice(-32.5).map((week, wIndex) => (
          <div key={wIndex} className="flex flex-col gap-1">
            {week.contributionDays.map((day) => (
              <motion.div
                key={day.date}
                className={`h-2.5 w-2.5 rounded-sm ${getColor(getIntensity(day.contributionCount))}`}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
      ...
      <div className="mt-2 flex items-center justify-end gap-2 text-xs text-neutral-500">
        ...
        <div className="flex gap-1">
          <div className="h-2.5 w-2.5 rounded-sm bg-neutral-200 dark:bg-neutral-800" />
          <div className="h-2.5 w-2.5 rounded-sm bg-purple-500/30" />
          <div className="h-2.5 w-2.5 rounded-sm bg-purple-500/70" />
